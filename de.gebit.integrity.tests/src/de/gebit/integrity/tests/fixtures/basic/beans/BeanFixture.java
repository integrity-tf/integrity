...
public class BeanFixture {
	...
	@FixtureMethod(description = "Returns a bean with nested date/time info")
	public DateContainingTestBean createDateContainingTestBean() {
		DateContainingTestBean.InnerBean tempInner1
				= new DateContainingTestBean.InnerBean(LocalDate.of(2023, 01, 01), LocalTime.of(18, 0, 0));
		DateContainingTestBean.InnerBean tempInner2 = new DateContainingTestBean.InnerBean(
				Date.from(ZonedDateTime.of(2022, 2, 2, 19, 30, 30, 0, ZoneId.systemDefault()).toInstant()));

		return new DateContainingTestBean(tempInner1, Arrays.asList(tempInner1, tempInner2));
	}
}
